...
def apply_my_classifier(model, xyxy, im0, labels):
    cutout = im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])]
    im = cv2.resize(cutout, (224, 224))  # BGR
    # cv2.imwrite('test%i.jpg' % j, cutout)
    ...
    label = model(torch.Tensor(im[np.newaxis])).argmax(1)  # classifier prediction
    #x[i] = x[i][pred_cls1 == pred_cls2]  # retain matching class detections
