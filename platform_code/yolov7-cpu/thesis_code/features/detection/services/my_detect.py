...
def detect(opt_source, 
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            #print('Detecion number {} in image in path {}'.format(i, path))
            #print('Det is {} with length {}'.format(det, len(det)))
            ...
            if len(det):
                ...
                print(f' Rescale boxes from img_size { img.shape[2:]} to im0 size {im0.shape}')
                print(f' Img0s with shape {im0s.shape}')
                
                det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()


                ## Print results the sum of labels found in Image
                ## print('Det unique is {} '.format(det[:, -1].unique()))
                ...
                for *xyxy, conf, cls in reversed(det):  
                    ...
                    if save_img or view_img:  # Add bbox to image                      
                        if apply_custom_classifier:
                          category = f'{names[int(cls)]}' # Determine the category
                         ...
                         # Check if the label is new for the category, if so initialize its count
                          ...
                          plot_one_box(xyxy, im0, label=label_new, color=colors[int(cls)], line_thickness=2)
                        else:
                          label = f'{names[int(cls)]} {conf:.2f}'
